      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <h4 className="text-sm font-medium text-slate-700">Cabinet Items</h4>
            <button
              type="button"
          ...
          {formData.items.length === 0 ? (
            <p className="text-sm text-slate-500 italic">No cabinet items added yet</p>
          ) : (
            <div className="space-y-2 mb-3">
              {formData.items.map((item, index) => (
                <div key={item.id || index} className="bg-slate-50 border border-slate-200 rounded-md p-3">
          ...
          {showItemForm && (
            <EstimateSectionItem
              item={editingItemIndex !== null ? formData.items[editingItemIndex] : {}}
              onSave={handleSaveItem}
              onCancel={handleCancelItemForm}
              onDelete={editingItemIndex !== null ? () => {
